-- Align receive_purchase_order return/update contract with the original (075):
--   * purchases.status := 'completed' on full receipt (not 'received')
--   * jsonb return includes 'status' = 'completed' | 'partial'
-- Keeps everything else from 20260909040000 (no updated_at on purchases,
-- partial raw receipts normalized, full-order journal on completion).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'items_received', v_rows,
    'fully_received', v_fully_received,
    'status', CASE WHEN v_fully_received THEN 'completed' ELSE 'partial' END
  );
END;
$function$;